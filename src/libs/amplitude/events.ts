...
export type AmplitudeTaxonomyEvents =
  ...
  // Non-standard event
  | { eventName: 'skjema steg startet'; data: { skjemanavn: string; steg: string /* skjemaId: number */ } }
  | {
      eventName: 'skjema steg spørsmål besvart'
      data: { skjemanavn: string; steg: string; spørsmål: string; svar: string /* skjemaId: number */ }
    }
  | { eventName: 'skjema avbrutt'; data: { skjemanavn: string; steg: string } }
  | { eventName: 'skjema innsending startet'; data: { skjemanavn: string /* skjemaId: number */ } }
